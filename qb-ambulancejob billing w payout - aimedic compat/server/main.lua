...
RegisterNetEvent('hospital:server:RevivePlayer', function(playerId, isOldMan)
	local src = source
	local billAmount = Config.AutoBilling
	local CutAmount = Config.CutAmount
	local Player = QBCore.Functions.GetPlayer(src)
	local Patient = QBCore.Functions.GetPlayer(playerId)
	...
	if Patient then
		if Player.PlayerData.job.name == 'ambulance' or QBCore.Functions.HasItem(src, 'firstaid', 1) then
			if oldMan then
				...
				if Patient.Functions.GetMoney('bank') >= billAmount then
					TriggerClientEvent('hospital:client:Revive', Patient.PlayerData.source)
					Patient.Functions.RemoveMoney('bank', billAmount, "revive-auto-billing")
					TriggerClientEvent('QBCore:Notify', Patient.PlayerData.source, "You were charged $" .. billAmount .. " for medical treatment", "error")

				Player.Functions.AddMoney('bank', CutAmount, 'medic-revived-player')
				TriggerClientEvent('QBCore:Notify', Player.PlayerData.source, "You were awarded $" .. CutAmount .. " for reviving a patient", "success")	
				else
					TriggerClientEvent('QBCore:Notify', Player.PlayerData.source, "Patient does not have enough money to cover treatment", "error")
        			TriggerClientEvent('QBCore:Notify', Patient.PlayerData.source, "You cannot afford the $" .. billAmount .. " revive fee, you must respawn", "error")
				end
			end
		else
