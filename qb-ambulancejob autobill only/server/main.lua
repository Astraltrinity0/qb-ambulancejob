...
-- Billing Function
RegisterNetEvent('billing:server:revivebill', function(Patient)
    local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	local Patient = QBCore.Functions.GetPlayer(playerId)
    local billAmount = Config.AutoBilling
    ...
    if Player.Functions.RemoveMoney("bank", billAmount, "revive-auto-billing") then
        TriggerClientEvent('QBCore:Notify', PlayerID, "You were charged $" .. billAmount .. " for medical treatment", "error")
    else
        TriggerClientEvent('QBCore:Notify', PlayerID, "You couldn’t afford the EMS bill", "error")
    end
end)
